...
const teachersModule = {
    ...
    actions: {
        ...
        async getTeachers({ commit }, idxPage) {
            try {
                const response = await axios.get(
                    `${urlEmployees}/filter?pageSize=${constants.pageSize}&pageNumber=${idxPage}`
                );
        ...
        async filterTeacher({ commit }, searchText) {
            try {
                const response = await axios.get(
                    `${urlEmployees}/filter?pageSize=${constants.pageSize}&pageNumber=${searchText.pageNumber}&employeeFilter=${searchText.content}`
                );
        ...
        async deleteTeacher({ commit }, teacherId) {
            try {
                await axios.delete(`${urlEmployees}/${teacherId}`);
                console.log('Xóa teacher: ', teacherId);
                commit('DELETE_TEACHER', teacherId);

                const response = await axios.get(
                    `${urlEmployees}/filter?pageSize=${
                        constants.pageSize
                    }&pageNumber=${1}`
                );
        ...
        async addNewTeacher({ commit }, newTeacher) {
            try {
                // await axios.post(`${urlEmployees}${newTeacher}`);
                await axios.post(urlEmployees, newTeacher, {
                    headers: {
